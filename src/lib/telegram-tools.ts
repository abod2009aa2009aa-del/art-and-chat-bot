...
const AI_TOOLS: Record<string, { sys: (arg: string) => string; ask: (arg: string) => string }> = {
  ...
  // ==================== فهم أعمق للمشاعر والنوايا + إبداع ====================
  emotion:    { sys: () => "أنت خبير تحليل نفسي لغوي. حلّل الحالة الشعورية خلف النص بعمق: المشاعر الظاهرة، المشاعر المخفية، مستوى التوتر/الإحباط (٠–١٠)، ونصيحة للتعامل مع صاحب النص. اذكر أدلة من الصياغة نفسها.", ask: (a) => `النص:\n\n${a}` },
  sarcasm:    { sys: () => "حدّد إن كان النص ساخراً أو جاداً أو تهكمياً أو مزاحاً، مع نسبة ثقة مئوية وشرح للإشارات اللغوية (مبالغة، تناقض، علامات ترقيم، سياق ثقافي عراقي/عربي).", ask: (a) => `النص:\n\n${a}` },
  intent:     { sys: () => "استخرج نية المتحدث الحقيقية خلف الكلام: الطلب الصريح، الطلب الضمني، الحاجة العاطفية، والرد الأمثل المقترح. كن مباشراً ومختصراً.", ask: (a) => `النص:\n\n${a}` },
  tone:       { sys: () => "حلّل نبرة النص (رسمي/ودّي/غاضب/بارد/متحمس…) واقترح إعادة صياغة بثلاث نبرات مختلفة.", ask: (a) => `النص:\n\n${a}` },
  empathy:    { sys: () => "اكتب رداً متعاطفاً إنسانياً حقيقياً يناسب حالة الشخص، بلا عبارات جاهزة باردة، بلهجة عربية دافئة.", ask: (a) => `الموقف: ${a}` },
  creative:   { sys: () => "أنت كاتب إبداعي بأسلوب فني مميز. أنتج نصاً إبداعياً أصيلاً (سرد/خاطرة/مشهد) بصور بلاغية قوية وبصمة أسلوبية خاصة، وابتعد عن الكليشيهات.", ask: (a) => `الفكرة: ${a}` },
  lyrics:     { sys: () => "اكتب كلمات أغنية بمقاطع (Verse/Chorus/Bridge) بإيقاع متسق وقافية طبيعية حسب الطلب.", ask: (a) => `الموضوع/الأسلوب: ${a}` },
  script:     { sys: () => "اكتب سيناريو مشهد درامي/كوميدي كامل: وصف المكان، الشخصيات، الحوار، والإرشادات الإخراجية.", ask: (a) => `المشهد: ${a}` },
  imagine:    { sys: () => "حوّل الوصف إلى Prompt احترافي لتوليد الصور (إنجليزي)، مفصّل بالإضاءة والعدسة والأسلوب والتكوين، ثم ترجمة عربية مختصرة.", ask: (a) => `الفكرة: ${a}` },
  memoryplan: { sys: () => "نظّم المعلومات المعطاة كذاكرة منظمة: بطاقات (موضوع → حقائق → قرارات → مهام مفتوحة) بصيغة مختصرة قابلة للاسترجاع السريع.", ask: (a) => `المحتوى:\n\n${a}` },
  apiplan:    { sys: () => "أنت مهندس تكاملات. صمّم خطة استدعاء API للمهمة: المسار، الطريقة، الرؤوس، جسم الطلب، معالجة الأخطاء، ومثال كود جاهز داخل بلوك ```FILE:integration.ts.", ask: (a) => `المهمة: ${a}` },
};
